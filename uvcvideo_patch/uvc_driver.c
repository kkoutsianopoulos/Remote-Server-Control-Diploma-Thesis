...
static struct uvc_format_desc uvc_fmts[] = {
	{
		.name 		= "YUV 4:2:2 (VYUY)",
		.guid		= UVC_GUID_FORMAT_VYUY,
		.fcc		= V4L2_PIX_FMT_VYUY,
	},
	{
		.name		= "YUV 4:2:2 (YUYV)",
		.guid		= UVC_GUID_FORMAT_YUYV,
		.fcc		= V4L2_PIX_FMT_YUYV,
	},
        {
                .name           = "YUV 4:2:2 (VYUY)",
                .guid           = UVC_GUID_FORMAT_VYUY2,
                .fcc            = V4L2_PIX_FMT_VYUY,
        },
        {
                .name           = "YUV 4:2:2 (YUYV)",
                .guid           = UVC_GUID_FORMAT_YUYV2,
                .fcc            = V4L2_PIX_FMT_YUYV,
        },
	{
		.name		= "YUV 4:2:2 (YUYV)",
...
static struct uvc_format_desc *uvc_format_by_guid(const u8 guid[16])
{
	...
	printk(KERN_DEBUG "Pixel Format array size=%d\n", len);
	printk(KERN_DEBUG "Comparing Formats");
	printk(KERN_DEBUG "GUID FROM DEVICE");
	for(j=0; j<16; j++){
		printk(KERN_CONT "%02X",guid[j]);
	}
	printk(KERN_DEBUG "==================================================");
	for (i = 0; i < len; ++i) {
		printk(KERN_DEBUG "Comparing: %d",i);
		printk(KERN_DEBUG "GUID FROM STRUCT");
		for(j=0; j<16; j++){
			printk(KERN_CONT "%02X", uvc_fmts[i].guid[j]);
		}
